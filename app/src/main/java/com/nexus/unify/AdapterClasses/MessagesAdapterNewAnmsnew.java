...
public class MessagesAdapterNewAnmsnew extends RecyclerView.Adapter<MessagesAdapterNewAnmsnew.MessageViewHolder> {
    ...
    public MessagesAdapterNewAnmsnew(Context context, List<MessageModel> messageList) {
    ...
    @NonNull
    @Override
    public MessagesAdapterNewAnmsnew.MessageViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull final MessagesAdapterNewAnmsnew.MessageViewHolder holder, int position) {
    ...
    String messageTime = message.getMessageTime().toString();
        ...
        if(fromUserId.equals(currentUserId)){
            ...
            try {
                holder.tvSentMessage.setText(AESDecryptionMethod(message.getMessage()));
            ...
            try {
                Glide.with(context)
                        .load(AESDecryptionMethod(message.getMessage()))
                        .placeholder(R.drawable.loadingavt)
                        .into(holder.ivSent);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
        ...
        else
        {
            ...
            try {
                holder.tvReceivedMessage.setText(AESDecryptionMethod(message.getMessage()));
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
        ...
        try {
            holder.clMessage.setTag(R.id.TAG_MESSAGE,AESDecryptionMethod( message.getMessage()));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        ...
        holder.clMessage.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                ...
                holder.clMessage.setBackgroundColor(context.getResources().getColor(com.varunjohn1990.iosdialogs4android.R.color.colorAccent));
    ...
    public  ActionMode.Callback actionModeCallBack = new ActionMode.Callback() {
        ...
        public boolean onCreateActionMode(ActionMode actionMode, Menu menu) {
           ...
           if (selectedMessageFrom!=firebaseAuth.getUid()){
        ...
        public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
            ...
            switch (itemId)
            {
                case  R.id.mnuDelete:
                    ...
                    if(context instanceof ChatsAnmsNewActivity)
                    {
                        ((ChatsAnmsNewActivity)context).deleteMessage(selectedMessageId, selectedMessageType);
                    }
                    actionMode.finish();
                    break;
                case  R.id.mnuDownload:
                    if(context instanceof  ChatsAnmsNewActivity)
                    {
                        ((ChatsAnmsNewActivity)context).downloadFile(selectedMessageId, selectedMessageType, false);
                    }
                    actionMode.finish();
                    break;
                case  R.id.mnuShare:
                    if(selectedMessageType.equals(Constants.MESSAGE_TYPE_TEXT)){
                        ...
                        try {
                            intentShare.putExtra(Intent.EXTRA_TEXT, AESDecryptionMethod(selectedMessage));
                        } catch (UnsupportedEncodingException e) {
                            e.printStackTrace();
                        }
                    ...
                    else
                    {
                        if(context instanceof ChatsAnmsNewActivity)
                        {
                            ((ChatsAnmsNewActivity)context).downloadFile(selectedMessageId, selectedMessageType, true);
                        }
                    }
                    actionMode.finish();
                    break;
                case  R.id.mnuForward:
                    ...
                    if(context instanceof  ChatsAnmsNewActivity)
                    {
                        ((ChatsAnmsNewActivity) context).forwardMessage(selectedMessageId, selectedMessage, selectedMessageType);
                    }
        ...
        public void onDestroyActionMode(ActionMode actionMode) {
            actionMode =null;
            selectedView.setBackgroundColor(context.getResources().getColor(R.color.light_bg));
